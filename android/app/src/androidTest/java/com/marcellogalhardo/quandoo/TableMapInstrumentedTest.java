...
public class TableMapInstrumentedTest {

    private static final int MOCK_WEB_SERVER_PORT = 1234;

    @Rule
    public ActivityTestRule<TableMapActivity> activityTestRule =
            new ActivityTestRule<>(TableMapActivity.class, false, true);
    ...
    public void shouldShowRetryWithSuccessWhenErrorHappens() throws IOException {
        TableMapActivity activity = activityTestRule.getActivity();
        server.enqueue(MockWebServerUtils.getGenericError());
        server.enqueue(MockWebServerUtils.getFake(activity, R.raw.table_map));
        ...
        onView(withId(R.id.recycler_view_tables))
                .check(matches(isDisplayed()));
    }
